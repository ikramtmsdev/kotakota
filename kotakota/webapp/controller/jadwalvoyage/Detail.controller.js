sap.ui.define([
    ...
    function (
        ...
        return BaseController.extend("sap.ui.kotakota.controller.contract.Detail", {
            formatter: formatter,

            onInit: async function () {
                this.oRouter = this.getOwnerComponent().getRouter();
                this.oRouter.getRoute("ContractDetail").attachPatternMatched(this._onPatternMatched, this);
            ...
            _onPatternMatched: async function (oEvent) {
                ...
                oView = this.getView();
                oView.setBusy(true);
                var JadwalVoyage = await this.AjaxGetData("Pelayaran(ID='" + ID.replace('/', '%2F') + "')");
                var oJadwalVoyage = new JSONModel(JadwalVoyage);
                this.getView().setModel(oJadwalVoyage);
                oView.bindElement({
                    path: "/"
                });
                oView.setBusy(false);
            ...
            postData: async function () {
                ...
                try {
                    await this.AjaxPatchData("Pelayaran(ID='" + ID.replace('/', '%2F') + "')", patch);
