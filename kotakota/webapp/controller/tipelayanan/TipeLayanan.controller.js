sap.ui.define([
    ...
    function (
        ...
        return BaseController.extend("sap.ui.kotakota.controller.tipelayanan.TipeLayanan", {
            formatter: formatter,

            onInit: function () {
                this.oRouter = this.getOwnerComponent().getRouter();
                this.oRouter.getRoute("TipeLayanan").attachPatternMatched(this._onPatternMatched, this);

            },

            _onPatternMatched: async function (oEvent) {
                // var TipeLayanan = this.getOwnerComponent().getModel("TipeLayanan");
                var TipeLayanan = await this.AjaxGetData("TipeLayanan");
                var oTipeLayanan = new JSONModel(TipeLayanan);
                this.getView().setModel(oTipeLayanan, "TipeLayanan");
                            ...
                            if (sAction == "OK") {
                                try {
                                    ...
                                    aIndices.forEach(async element => {
                                        ...
                                        await ini.AjaxDeleteData("TipeLayanan(ID='" + ID + "')");
                                    });
                                } catch (error) {
            ...
            onAdd: function (oEvent) {
                this.getOwnerComponent().getRouter().navTo("TipeLayananAdd");
            },
            ...
            handleActionPress: function (oEvent) {
                ...
                var ID = this.getView().getModel("TipeLayanan").getProperty("ID", oRow.getBindingContext("TipeLayanan"));
                if (oAction == "Delete") {
                    this.onDelete(ID);
                }
                else if (oAction == "Detail") {
                    this.getOwnerComponent().getRouter().navTo("TipeLayananDetail", { id: ID });
                }
            },
