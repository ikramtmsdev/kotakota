sap.ui.define([
    ...
    function (
        ...
        return BaseController.extend("sap.ui.kotakota.controller.zone.Zone", {
            formatter: formatter,

            onInit: function () {
                this.oRouter = this.getOwnerComponent().getRouter();
                this.oRouter.getRoute("Zone").attachPatternMatched(this._onPatternMatched, this);

            },

            _onPatternMatched: async function (oEvent) {
                // var Zone = this.getOwnerComponent().getModel("Zone");
                var Zone = await this.AjaxGetData("ZoneData");
                            ...
                            if (sAction == "OK") {
                                try {
                                    ...
                                    aIndices.forEach(async element => {
                                        ...
                                        await ini.AjaxDeleteData("Zone(ID='" + ID + "')");
                                    });
                                } catch (error) {
            ...
            onAdd: function (oEvent) {
                this.getOwnerComponent().getRouter().navTo("ZoneAdd");
            },
            ...
            handleActionPress: function (oEvent) {
                ...
                var ID = this.getView().getModel("Zone").getProperty("ID", oRow.getBindingContext("Zone"));
                if (oAction == "Delete") {
                    this.onDelete(ID);
                }
                else if (oAction == "Detail") {
                    this.getOwnerComponent().getRouter().navTo("ZoneDetail", { id: ID });
                }
            },
