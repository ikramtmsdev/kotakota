sap.ui.define([
    ...
    function (
        ...
        return BaseController.extend("sap.ui.kotakota.controller.contract.Contract", {
            formatter: formatter,

            onInit: function () {
                this.oRouter = this.getOwnerComponent().getRouter();
                this.oRouter.getRoute("Contract").attachPatternMatched(this._onPatternMatched, this);

            },

            _onPatternMatched: async function (oEvent) {
                // var Contract = this.getOwnerComponent().getModel("Contract");
                var Contract = await this.AjaxGetData("Contract");
                var oContract = new JSONModel(Contract);
                this.getView().setModel(oContract, "Contract");
                            ...
                            if (sAction == "OK") {
                                try {
                                    ...
                                    aIndices.forEach(async element => {
                                        ...
                                        await ini.AjaxDeleteData("Contract(ID='" + ID + "')");
                                    });
                                } catch (error) {
            ...
            onAdd: function (oEvent) {
                this.getOwnerComponent().getRouter().navTo("ContractAdd");
            },
            ...
            handleActionPress: function (oEvent) {
                ...
                var ID = this.getView().getModel("Contract").getProperty("ID", oRow.getBindingContext("Contract"));
                if (oAction == "Delete") {
                    this.onDelete(ID);
                }
                else if (oAction == "Detail") {
                    this.getOwnerComponent().getRouter().navTo("ContractDetail", { id: ID });
                }
            },
