sap.ui.define([
    ...
    function (
        ...
        return BaseController.extend("sap.ui.kotakota.controller.partner.Partner", {
            formatter: formatter,

            onInit: function () {
                this.oRouter = this.getOwnerComponent().getRouter();
                this.oRouter.getRoute("Partner").attachPatternMatched(this._onPatternMatched, this);

            },

            _onPatternMatched: async function (oEvent) {
                // var Partner = this.getOwnerComponent().getModel("Partner");
                var Partner = await this.AjaxGetData("Partner");
                var oPartner = new JSONModel(Partner);
                this.getView().setModel(oPartner, "Partner");
                            ...
                            if (sAction == "OK") {
                                try {
                                    ...
                                    aIndices.forEach(async element => {
                                        ...
                                        await ini.AjaxDeleteData("Partner(ID='" + ID + "')");
                                    });
                                } catch (error) {
            ...
            onAdd: function (oEvent) {
                this.getOwnerComponent().getRouter().navTo("PartnerAdd");
            },
            ...
            handleActionPress: function (oEvent) {
                ...
                var ID = this.getView().getModel("Partner").getProperty("ID", oRow.getBindingContext("Partner"));
                if (oAction == "Delete") {
                    this.onDelete(ID);
                }
                else if (oAction == "Detail") {
                    this.getOwnerComponent().getRouter().navTo("PartnerDetail", { id: ID });
                }
            },
