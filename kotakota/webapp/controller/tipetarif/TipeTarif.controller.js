sap.ui.define([
    ...
    function (
        ...
        return BaseController.extend("sap.ui.kotakota.controller.tipetarif.TipeTarif", {
            formatter: formatter,

            onInit: function () {
                this.oRouter = this.getOwnerComponent().getRouter();
                this.oRouter.getRoute("TipeTarif").attachPatternMatched(this._onPatternMatched, this);

            },

            _onPatternMatched: async function (oEvent) {
                // var TipeTarif = this.getOwnerComponent().getModel("TipeTarif");
                var TipeTarif = await this.AjaxGetData("TipeTarif");
                var oTipeTarif = new JSONModel(TipeTarif);
                this.getView().setModel(oTipeTarif, "TipeTarif");
                            ...
                            if (sAction == "OK") {
                                try {
                                    ...
                                    aIndices.forEach(async element => {
                                        ...
                                        await ini.AjaxDeleteData("TipeTarif(ID='" + ID + "')");
                                    });
                                } catch (error) {
            ...
            onAdd: function (oEvent) {
                this.getOwnerComponent().getRouter().navTo("TipeTarifAdd");
            },
            ...
            handleActionPress: function (oEvent) {
                ...
                var ID = this.getView().getModel("TipeTarif").getProperty("ID", oRow.getBindingContext("TipeTarif"));
                if (oAction == "Delete") {
                    this.onDelete(ID);
                }
                else if (oAction == "Detail") {
                    this.getOwnerComponent().getRouter().navTo("TipeTarifDetail", { id: ID });
                }
            },
